<?php
    ...
    if(isset($_REQUEST['id'])) {
    	...
    	$id_kasir = $_REQUEST['id'];
    	extract($pengurus->getPengurusID($id_kasir));
    }
		            ...
		            <div class="container-fluid">
		                <div class="row">
		                	<div class="col-md-12 card">
	                			<div class="header">
	                                <h4 class="title">Laporan Penjualan Kasir <font color="Red">"<?php echo $nama ?>"</font></h4>
		                                ...
		                                <tbody>
		                                	<?php
		                                		if(!$_POST['tgl_awal'] == NULL && !$_POST['tgl_akhir'] == NULL) {
		                                			...
		                                			$query = "SELECT *, SUM(order_detail_kasir.jumlah_produk) as total, 
												SUM(order_detail_kasir.total_harga) as harga FROM order_detail_kasir INNER JOIN order_kasir 
												INNER JOIN produk ON (order_detail_kasir.id_ok = order_kasir.id_ok) AND (order_detail_kasir.id_produk = produk.id_produk) 
												WHERE date(order_kasir.tgl_order) BETWEEN '{$tgl_awal}' AND '{$tgl_akhir}' AND order_kasir.id_pengurus={$id_kasir} GROUP BY produk.kode_SKU";       
													$records_per_page=15;
													$newquery = $produk->paging($query,$records_per_page);
													$produk->laporanPerKasir($newquery);
